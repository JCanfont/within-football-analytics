...
describe("App", () => {
  ...
  it("opens Flashscore from the left menu and captures 1X2 odds on demand", async () => {
    ...
    expect(screen.getByText(/Pendiente de inicio|Esperando inicio real/)).toBeInTheDocument();
  ...
  it("emails once when Flashscore Ultra shows the low-odds team scored before minute 30", async () => {
    ...
    mockedApi.fetchFlashscoreMatches.mockResolvedValue({
      provider: "flashscore",
      status: "ok",
      message: "1 favorito ≤ 1.60 vigilable.",
      configured: true,
      threshold: 1.6,
      alert_threshold: 1.5,
      matches: [{
    ...
    expect(await screen.findByText("Esperando inicio real")).toBeInTheDocument();
    await waitFor(() => {
